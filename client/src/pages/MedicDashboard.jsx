    ...
    try {
      ...
      setError(
        loadError.response?.data?.message ||
          "Unable to load the medic workspace right now."
      );
    } finally {
        ...
        <div className="rounded-3xl border border-slate-100 bg-white p-5 shadow-sm">
          ...
          <h2 className="mt-3 text-xl font-bold text-slate-900">
            {profile?.approvedRole
              ? profile.approvedRole.replaceAll("_", " ")
      ...
      <section className="mt-8 rounded-3xl border border-slate-100 bg-white p-6 shadow-sm">
        ...
        <div className="mt-6 grid gap-4 md:grid-cols-2 xl:grid-cols-3">
          ...
          {recentCases.length === 0 && (
            <div className="rounded-3xl border border-dashed border-slate-200 p-8 text-center text-slate-500 md:col-span-2 xl:col-span-3">
...
const MedicStatCard = ({ title, value, icon, tone, subtext }) => (
  ...
  return (
    <div className="rounded-3xl border border-slate-200 p-5">
      <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
        <div>
          <div className="flex flex-wrap items-center gap-3">
            <h3 className="text-lg font-bold text-slate-900">{caseItem.patient.name}</h3>
      ...
      <div className="mt-4 rounded-2xl bg-slate-50 p-4">
        <p className="text-xs font-semibold uppercase tracking-[0.18em] text-slate-400">Symptoms</p>
...
const CaseSummaryTile = ({ caseItem }) => (
  <div className="rounded-3xl border border-slate-200 bg-slate-50 p-4">
    <div className="flex items-start justify-between gap-3">
      <div>
        <div className="text-sm font-semibold text-slate-900">{caseItem.patient.name}</div>
        <div className="mt-1 text-xs text-slate-500">{formatDate(caseItem.createdAt)}</div>
      </div>
